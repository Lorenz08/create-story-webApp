...
(() => {
	const storyPath = path.join(__dirname, '../statusFiles');
	fs.rmdirSync(storyPath, {recursive: true}); 
	/*if(fs.existsSync(storyPath))
        fs.readdirSync(storyPath).forEach((file) => {
            fs.rmSync(storyPath+file);                    
        });

	let storyPath2 = path.join(__dirname, '../statusFiles/Simone_0/');
	if(fs.existsSync(storyPath2))
        fs.readdirSync(storyPath2).forEach((file) => {
            fs.rmSync(storyPath2+file);                    
        })

	storyPath2 = path.join(__dirname, '../statusFiles/Simone_1/');
	if(fs.existsSync(storyPath2))
        fs.readdirSync(storyPath2).forEach((file) => {
            fs.rmSync(storyPath2+file);                    
        })
	...
	storyPath2 = path.join(__dirname, '../statusFiles/Simone_2/');
	if(fs.existsSync(storyPath2))
        fs.readdirSync(storyPath2).forEach((file) => {
            fs.rmSync(storyPath2+file);                    
        })

	storyPath2 = path.join(__dirname, '../statusFiles/Luca_0/');
	if(fs.existsSync(storyPath2))
        fs.readdirSync(storyPath2).forEach((file) => {
            fs.rmSync(storyPath2+file);                    
        })*/
