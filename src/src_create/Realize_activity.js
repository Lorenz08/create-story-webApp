...
function Realize_activity(props){
    ...
    function changeLetter(string){
        ...
        res = res.replace("A`", '\xc0')
        res = res.replace("E`", '\xc8')
        res = res.replace("E´", '\xc9')
        res = res.replace("I`", '\xcc')
        res = res.replace("O`", '\xd2')
        res = res.replace("U`", '\xc9')
        return res
    };
        ...
        if (activity.title == "") displayDialog("Inserire prima un titolo");
            ...
            if (activity.widgetType == "Quattro opzioni" && activity.fourAnswers.length < 4) displayDialog("Inserire prima esattamente 4 risposte se si vuole scegliere questo tipo di risposta")
                ...
                if (imageBackground != null){
                    await axios.post(`http://site181997.tw.cs.unibo.it/addImage/${props.story.id}/${title}_background`, imageBackground, {
                ...
                if (imageActivity != null){
                    await axios.post(`http://site181997.tw.cs.unibo.it/addImage/${props.story.id}/${title}_activity`, imageActivity, {
                        headers:{ "Content-Type": "multipart/form-data" }
                    })
                    .then((response) => {
                    ...
                    if (activity.activityImage != ""){
                        await axios.get(`http://site181997.tw.cs.unibo.it/duplyImage/${activity.activityImage}/${props.story.id}_${title}_activity`)
                        .then((response) => {
                            activityImageTmp = response.data
                            setActivity({...activity, ["activityImage"]: response.data});
                        })
                ...
                if (isDeleteBackgroundImage){
                    await axios.delete(`http://site181997.tw.cs.unibo.it/deleteImage/${activity.backgroundImage}`)
                    .then(() => {
                        setIsDeleteBackgroundImage(false)
                        backgroundImageTmp = ""
                        setActivity({...activity, ["backgroundImage"]: ``});
                ...
                if (isDeleteActivityImage){
                    await axios.delete(`http://site181997.tw.cs.unibo.it/deleteImage/${activity.activityImage}`)
                    .then(() => {
                        setIsDeleteActivityImage(false)
                        activityImageTmp = ""
                        setActivity({...activity, ["activityImage"]: ``});
