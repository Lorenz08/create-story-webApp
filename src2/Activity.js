...
import {getFinalMessage,correctAnswerAction,wrongAnswerAction,mustAnswer,checkButton} from './utilsActivity.js'
...
export const Activity = React.forwardRef((props, ref) => {
    ...
    function inc( path ){ 
        if( mustAnswer(actual) || (lastAnswer != null)){
            ...
            if(counter != 0){
                   ...
                   if(dinamicActivities[counter- 1].activityImage != ""){ setImg("");}
		    now = new Date();
                    ...
                    switch(dinamicActivities[counter].widgetType){
                        ...
                        break;
                        case "Vero o falso":
			   const falsePoints = eval(dinamicActivities[counter].trueFalseAnswer.falseScore);
                           	const truePoints = eval(dinamicActivities[counter].trueFalseAnswer.trueScore);     	
				...
				props.setPoints(props.points + ((truePoints > 0)? truePoints : falsePoints));                               
				                actualPoints = (truePoints > 0)? truePoints : falsePoints;

                            }else{
                                wrongAnswerAction(props.playerId,props.story,props.socket,dinamicActivities,counter,props.dictionaryActivity,activities,actual);
         		 
				props.setPoints(props.points +( (truePoints > 0)? falsePoints : truePoints));                                        
				actualPoints = (truePoints > 0)? falsePoints : truePoints;
                            }
			...
			if(eval(dinamicActivities[counter].multipleAnswers[answer].score)>0){
				...
				const text = document.getElementById("textAnswer").value;
                            if( text.toLowerCase() === dinamicActivities[counter].textAnswer.value.toLowerCase()){
                                correctAnswerAction(props.playerId,props.story,props.socket,dinamicActivities,counter,props.dictionaryActivity,activities,actual);
	    ...
	    else if(dinamicActivities[counter] === props.json.lastActivity) {
    ...
    axios.get(`http://site181997.tw.cs.unibo.it/downloadImage/${dinamicActivities[counter].activityImage}`, { responseType:"blob" })
    ...
    if(img !== 0)
      ...
      if(dinamicActivities[counter].streamVideo !== ""){
        ...
        }else if(videoSource.indexOf('youtu.be')>-1){
	...
	const videos = {marginTop:'10px'};
	mediaProp.push(e("br",null));
        mediaProp.push(e("iframe",{controls: true ,key:`video${counter}`,style:videos, align:'center',id:'ytplayer',allowFullScreen:true,autoPlay: true,src:source,key:"video"}));
    }
