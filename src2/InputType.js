...
function inputType(props){
    ...
    inputElement.push(e("textarea",{'aria-labelledby' : "activitIntro", id:"textAnswer", key:"input", style:styl}));
        ...
        inputElement.push(
                ...
                const uploadFile = () => {
                    file != '' ? props.setDisabled(true): null;
                    const formData = new FormData();        
                    formData.append('file', file); // appending file
                    axios.post('http://site181997.tw.cs.unibo.it/uploadImg', formData).then(res => {
                            if(res.status == 200)   
                                props.socket.emit("send-humanEvaluation",{question: props.v[props.counter].question, answer: 'http://site181997.tw.cs.unibo.it/server/' + res.data.path, type : "image" , id : props.playerId, section : props.counter});       
                                props.inc('http://site181997.tw.cs.unibo.it/server/' + res.data.path);
        ...
        return e("div",null, 
                    e("div", {key:`inputElement${props.counter}` , style:inputGroup}, inputElement),
                    e("button", {role: "button", key:`buttonNext${props.counter}`,id: "nextButton",style:props.btnNext,onClick: props.inc}, "SUCCESSIVO")
                );
}
